...
**VERIFIED, found during session closeout (after this document was first written):** the `776 - Build Log and Decisions` folder holds far more prior audit material than assumed when this dossier and the audit plan were first written — `Phil OS - Psychometric Validity Audit.md`, `Phil OS - Psychometric Validity Audit v2 Post-Fix.md`, `Phil OS - Full Psychometric and Philosophical Audit v3.md`, `Phil OS - Item Analysis and Question Standards.md`, `Phil OS - Question Bank v2 Cross-Check Report.md`, `Phil OS - Audit Findings vs Q Bank v2 Compliance.md`, `Phil OS - Council Analysis Summary.md`. Multiple audit rounds already happened. Read these before assuming Phase 2 is a blank-slate build. Also confirmed: there is no `Phil OS - Logic Log.md` file anywhere in the vault (searched directly) — `CLAUDE.md` referenced one that never existed; scoring/logic changes actually belong in `Phil OS - Scoring and Archetype Logic.md`, which itself lives in `776 - Build Log and Decisions`, not `772 - Architecture and Build` as `CLAUDE.md`'s file tree previously claimed. Both corrected in `CLAUDE.md` during closeout.

**NOT VERIFIED / NEEDS FABLE:**
- Whether the vault docs listed above actually match current code line-by-line — not opened/diffed this session; Phase 1 as scoped in the audit plan still needs a full read-and-diff pass against them specifically.
...
**10i (mobile sign-in bug) / mobile dropdown scroll bug:** Investigated structurally, root cause not found — both need live device/browser reproduction, not static reading. See fixes log for the specific suspect code regions (the `#auth-modal` pseudo-element background system; the `#report-nav-dropdown` and its parent scroll container). **Found during closeout:** Decisions Log D17 and D24 already fixed two related bugs on this exact same modal (GPU-compositing content vanishing during scroll on Chrome/Android; a dead-CSS-selector ghosting bug). Check whether Andre's current tap-triggered report is a regression of either before treating it as unaddressed.
