...
// ---- 13. Inertness: nothing in the product imports this module ----
{
  const importers = [];
  const roots = ['api', 'lib', '.'];
  roots.forEach((dir) => {
    fs.readdirSync(path.join(__dirname, '..', dir)).forEach((f) => {
      ...
      if (src.includes('report-output-validation') && !full.endsWith('report-output-validation.js')) {
        importers.push(path.join(dir, f));
      }
    });
  });
  ok('module is inert: no product file references report-output-validation', importers.length === 0, importers);
}
