...
// as a qualifier anywhere in the registry). Scans all five text fields, not
// just qualifier - a 2026-08-02 pre-wiring review (B-1) found the same class
// of leak ("...which is why this placement needed explicit D156 approval")
// sitting inside two diverge strings, which this narrower qualifier-only
// scan did not catch; fixed at the vault-doc source and re-extracted.
{
  ...
  VARIANT_IDS.forEach((id) => {
    FAMOUS_MINDS_REGISTRY[id].figures.forEach((f) => {
      if (f.qualifier && EDITORIAL_RE.test(f.qualifier)) leaked.push(`${id}/${f.name}.qualifier: ${f.qualifier}`);
      TEXT_FIELDS.forEach((field) => {
        if (EDITORIAL_RE.test(f[field])) leaked.push(`${id}/${f.name}.${field}: ${f[field]}`);
      });
    });
  });
  ok('no editorial/decision-tracking language leaked into any qualifier or content field', leaked.length === 0, leaked);
