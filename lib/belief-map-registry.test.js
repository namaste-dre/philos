...
// ---- 1. Registry shape and validator agreement ----
{
  ...
  ok('registry holds all 32 axes (full content layer as of 2026-07-31)', ids.length === 32, ids.length);
...
// ---- 6. Glossary (content standard Part V, spec Section 10) ----
{
  const { GLOSSARY } = reg;
  ok('GLOSSARY is exported and frozen', GLOSSARY && Object.isFrozen(GLOSSARY));
  ok('GLOSSARY holds exactly 32 terms', Object.keys(GLOSSARY).length === 32, Object.keys(GLOSSARY).length);

  const em = String.fromCharCode(0x2014), en = String.fromCharCode(0x2013);
  const dirty = Object.entries(GLOSSARY).filter(([t, g]) =>
    t.includes(em) || t.includes(en) || g.includes(em) || g.includes(en));
  ok('no em/en dash in any glossary term or gloss', dirty.length === 0, dirty.map(d => d[0]));

  const empty = Object.entries(GLOSSARY).filter(([, g]) => !g || g.trim().length < 20);
  ok('every gloss is a real one-line definition (20+ chars)', empty.length === 0, empty.map(d => d[0]));

  // Terms the band texts actually use should be glossed: spot-check the
  // ones the content standard names as load-bearing.
  const needed = ['compatibilism', 'naturalism', 'physicalism', 'sentience', 'the hard problem'];
  const missing = needed.filter(t => !(t in GLOSSARY));
  ok('load-bearing terms present in the glossary', missing.length === 0, missing);
}

// ---- 7. Vault-doc parity: the canonical content doc is the source of ----
// truth; every string this registry serves must appear in it verbatim.
// The doc lives outside the repo (Andre's Obsidian vault), so this
// section SKIPs - loudly, without failing - on machines without it.
// On the authoring machine it makes doc-to-code drift impossible to miss
// (the same DI-005 principle registry-parity.test.js applies client/server).
{
  const DOC = process.env.BELIEF_MAP_DOC ||
    "C:/Andre's 2nd brain/750 - Other Ventures/757 - Phil OS/Build Log and Decisions/Phil OS - Belief Map Content Standard and Exemplar Axes.md";
  if (!fs.existsSync(DOC)) {
    ...
    for (const [id, e] of Object.entries(BELIEF_MAP_REGISTRY)) {
      const fields = {
        expandedDefinition: e.expandedDefinition,
        commonMisconception: e.commonMisconception,
        'poles.L.shortDef': e.poles.L.shortDef,
        'poles.L.expandedDef': e.poles.L.expandedDef,
        'poles.R.shortDef': e.poles.R.shortDef,
        'poles.R.expandedDef': e.poles.R.expandedDef,
      };
      for (const b of BAND_KEYS) {
        fields['bands.' + b + '.full'] = e.bands[b].full;
        fields['bands.' + b + '.short'] = e.bands[b].short;
      }
      for (const [p, v] of Object.entries(fields)) {
        if (typeof v === 'string' && v.length > 0 && !doc.includes(v)) missing.push(id + '.' + p);
      }
    }
    ok('every registry text field appears verbatim in the vault content doc (' +
      (Object.keys(BELIEF_MAP_REGISTRY).length * 16) + ' fields)', missing.length === 0, missing.slice(0, 8));

    const { GLOSSARY } = reg;
    const gMissing = Object.entries(GLOSSARY)
      .filter(([t, g]) => !doc.includes('- **' + t + ':** ' + g))
      .map(([t]) => t);
    ok('every glossary entry appears verbatim in the vault content doc', gMissing.length === 0, gMissing);
  }
}
