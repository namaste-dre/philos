...
- Verify claims against LIVE code (`C:\philos\index.html`, `api/*.js`) and the LIVE Supabase project (`Phil/OS`, id `qzazpihvayeqjfgtbakr` - MCP access works), never against docs alone. The card is 1080x1920. The model is claude-sonnet-5. There are exactly 42 real contradiction rules _(correction 2026-07-06, post-handover: the split is now 11A/22B/9C after D54's interim C01b demote, and the phantom array slot is fixed and deployed - .length reads 42)_.
